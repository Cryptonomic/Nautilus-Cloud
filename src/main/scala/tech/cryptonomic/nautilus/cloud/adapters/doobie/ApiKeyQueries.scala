...
trait ApiKeyQueries extends EnvironmentMappers {
  ...
  def getUserKeysValidIn(userId: UserId, begin: Instant, end: Instant): Query0[ApiKey] =
    sql"SELECT keyid, key, environment, userid, dateissued, datesuspended FROM api_keys WHERE userid = $userId AND ((dateissued >= $begin AND dateissued <= $end) OR (datesuspended >= $begin AND datesuspended <= $end))"
      .query[ApiKey]
