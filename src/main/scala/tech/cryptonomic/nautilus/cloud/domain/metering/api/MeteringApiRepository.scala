...
trait MeteringApiRepository[F[_]] {
  ...
  def getApiKey5mStats(apiKeys: List[ApiKey], from: Option[Long] = None): F[Result[List[ApiKeyStats]]]

  /** Fetches ApiKey stats per 24h */
  def getApiKey24hStats(apiKeys: List[ApiKey], from: Option[Long] = None): F[Result[List[ApiKeyStats]]]

  /** Fetches Route stats per 5m */
  def getRoute5mStats(apiKeys: List[ApiKey], from: Option[Long] = None): F[Result[List[RouteStats]]]

  /** Fetches Route stats per 24h */
  def getRoute24hStats(apiKeys: List[ApiKey], from: Option[Long] = None): F[Result[List[RouteStats]]]

  /** Fetches IP stats per 5m */
  def getIp5mStats(apiKeys: List[ApiKey], from: Option[Long] = None): F[Result[List[IpStats]]]

  /** Fetches IP stats per 24h */
  def getIp24hStats(apiKeys: List[ApiKey], from: Option[Long] = None): F[Result[List[IpStats]]]
}
